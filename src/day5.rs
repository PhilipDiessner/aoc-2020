...
pub fn part1(filename: &str) {
    ...
    for pass in passes {
        ...
        ids.push(u32::from_str_radix(row.as_str(),2).unwrap()*8+
            u32::from_str_radix(col.as_str(),2).unwrap());
    }
    ids.sort();
    ...
    let mut spot = start;
    for id in ids {
        if start == id { start += 1 } else { spot = start; break; }
    }
    println!("{}, {}",max, spot);
}
